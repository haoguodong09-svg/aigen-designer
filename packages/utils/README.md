...
Aigen Designer 核心工具库，提供通用工具函数、页面管理器、插件管理器等核心功能模块。

## 📦 安装

```bash
npm install @aigen-designer/utils
# 或
yarn add @aigen-designer/utils
# 或
pnpm add @aigen-designer/utils
```
...
#### 字符串工具
```typescript
import { capitalizeFirstLetter, getFileNameByUrl } from '@aigen-designer/utils';
...
#### 异步组件加载
```typescript
import { loadAsyncComponent } from '@aigen-designer/utils';
...
```typescript
import { usePageManager } from '@aigen-designer/utils';
...
```typescript
import { pluginManager } from '@aigen-designer/utils';
...
```typescript
import { useRevoke } from '@aigen-designer/utils';
